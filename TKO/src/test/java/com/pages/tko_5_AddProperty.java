...
public class tko_5_AddProperty {
	...
	public SelenideElement agent() {
		return $(By.xpath("//div[@class='v-table-cell-wrapper' or @class='v-captiontext']")).waitUntil(visible,
				app.timeOut);
	}

	public SelenideElement menuProperty() {
		return $(By.xpath("//div[@class='v-captiontext' and text()='Объекты недвижимости']")).waitUntil(visible,
				app.timeOut);
	}
	...
	public SelenideElement TextBoxKadasNumber() {
		return $(By.xpath("//div[@class='v-widget v-has-caption v-caption-on-right v-has-width']/input"))
				.waitUntil(visible, app.timeOut);
	}
...
//Указываем кадастровый номер
		...
		sleep(1000);
//Нажимаем "Найти" (Появляются доп. поля)
		buttonFind().click();
		sleep(1000);
...
//		 System.out.println(nameTextEtap); Assert.assertEquals(nameTextEtap,
//		 "Верификация");
//		 
